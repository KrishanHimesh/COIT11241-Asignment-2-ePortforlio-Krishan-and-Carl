...
function PreTest {
    ...
    Write-Output "Unauthorised app Keyfinder is running"
...
function SetupWDAC {
    ...
    if ($null -ne $WDACPolicy.SiPolicy.PolicyID) {
    ...
    Write-Output "WDAC Policy Applied Successfully"
}
...
function ResetWDAC {
    ...
    Write-Output "Attempting to remove $PolicyPath..."
    if (Test-Path $PolicyPath) {
        Remove-Item -Path $PolicyPath -Force -ErrorAction Continue
    }

    Write-Output "WDAC Policy Reset Successfully. Please restart your PC or it will restart automatically in 10 serconds"
...
switch ($command) {
    ...
    default {
        Write-Output "Enter the commamd"
    }
}
